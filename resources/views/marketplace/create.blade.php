...
@section('content')
    <div class="container page-container">
        ...
        <div class="row justify-content-center">
            <div class="col-md-9">
                <div class="card">
                    @include('partials.card-header', ['title' => trans('marketplace.create_product')])
                    <div class="card-body">
                        @include('partials.message')
                        <form action="{{ route('marketplace.store') }}" method="post">
                            @csrf
                            @livewire('market-place.create', ['product' => null, 'is_promoted' => true])
                            ...
                            <div class="row d-none" id="validatedFields">
                                <div class="col-md-12">
                                    ...
                                    @include('partials.payment-livewire', [
                                        ...
                                        'type' => 'product',
                                        'fee' => number_format(
                                            convert_price(support_setting('marketplace_add_product_price')),
                                            2),
                                        'is_disable' => false,
                                    ])
                                </div>
                                <div class="col-md-12 text-center mt-3">
                                    <button type="submit" id="submitBtn" class="btn btn-genix"
                                        disabled>{{ trans('general.create') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
@push('scripts')
    <script>
        window.addEventListener('dataValidated', () => {
