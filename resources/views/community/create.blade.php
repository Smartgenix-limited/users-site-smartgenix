...
@section('content')
    <div class="container page-container">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    @include('partials.card-header', ['title' => 'Ask Question'])
                    <div class="card-body">
                        ...
                        <form action="{{ route('communities.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="message" class="form-control @error('message') is-invalid @enderror" name="message"
                                    placeholder="{{ __('community.message') }}" rows="7" required>{{ old('message') }}</textarea>
                                @error('message')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div id="submitBtn" class="text-center mt-4">
                                <button type="submit" class="btn btn-primary"
                                    {{ !request()->user()->can_community ? 'disabled' : '' }}>{{ trans('general.submit') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
