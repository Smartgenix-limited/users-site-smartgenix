                                ...
                                @if ($mot->status === 'completed')
                                    ...
                                    @if ($mot->mot_task->mot_status !== 'pass')
                                        <div class="text-center mt-3">
                                            <a href="{{ route('services.create') }}"
                                                class="btn btn-sm btn-primary">{{ trans('general.book_service') }}</a>
                                    ...
                                    @foreach ($mot->mot_task->tasks as $task)
                                        <div class="d-flex mb-2">
                                            <span class="h6">{{ trans('general.task_name') }}:</span>
                                            <span class="h6 pl-3">{{ $task['name'] }}</span>
                                        </div>
                                        <div class="d-flex mb-2">
                                            <span class="h6">{{ trans('general.task_result') }}:</span>
                                            <span
                                                class="h6 pl-3 text-capitalize @if ($task['result'] === 'pass') text-success @else text-danger @endif">{{ $task['result'] }}</span>
                                        </div>
                                        @if ($task['result'] !== 'pass')
                                            <div class="d-flex mb-2">
                                                <span class="h6">{{ trans('general.fail_reason') }}:</span>
                                                <span class="h6 pl-3">{{ $task['reason'] }}</span>
                                            </div>
                                        @endif
                                        <hr>
                                    @endforeach
